...
Args=[--deploy-mode,cluster,--name,als-c34xlarge-2-3-5-40,--num-executors,6,--executor-cores,5,--executor-memory,6200m,--conf,spark.executor.extraJavaOptions='-XX:ThreadStackSize=2048',--conf,spark.sql.shuffle.partitions=40,--conf,spark.default.parallelism=40,--class,${APPMAP[$app]},$s3_jar_path,$app,$s3_folder_path] | python2.7 scripts/emr_adhoc.py